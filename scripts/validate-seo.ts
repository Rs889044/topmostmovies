...
async function main() {
  ...
  for (const file of files) {
    ...
    if (page === '/') {
      ...
      /^\/(industry|country|language|genre|studio|theme|year|decade)\/.+/.test(page) ||
      /^\/best\/.+/.test(page) ||
      /^\/(best-movies|popular|mature)$/.test(page)
    ) {
    ...
    } else if (/^\/(industry|country|language|genre|studio|theme|year|decade)$/.test(page)) {
